...
This project is not just a "cloud-ready" application; it was designed from the ground up as a **cloud-native** service. It is built to be deployed, scaled, and managed in a modern cloud environment like [Kubernetes](https://kubernetes.io/).

Here are the core cloud-native principles this project follows:

* **1. Containerization:** The application is packaged as a lightweight, standalone container using the provided `Dockerfile`. This is automated by the [Docker Publish workflow](https://www.google.com/search?q=.github/workflows/docker-publish.yml).

* **2. Kubernetes-Native:** The entire application is designed to be run and orchestrated by Kubernetes. The [`/deployments/go-testing`](https://www.google.com/search?q=deployments/go-testing) directory contains all the necessary manifests, including:

    * **Declarative `Deployment`:** A `deployment.yml` file defines the desired state.
    * **GitOps-Ready Secrets:** Uses **Sealed Secrets** (`sealed-app-secrets-go-geo-tree-table.yaml`) for safely storing sensitive configuration in Git.
    * **Configuration Decoupling:** Follows 12-Factor App principles by separating non-sensitive config into a `ConfigMap` and sensitive data into `Secrets`.

* **3. Stateless (12-Factor App):** The service is stateless and configured entirely via environment variables, which are injected by the K8s `ConfigMap` and `Secret`. This allows it to be scaled horizontally with ease.

* **4. Observability:** The application provides `/health` and `/readiness` endpoints. This is crucial for a cloud environment, allowing Kubernetes to automatically manage the application's lifecycle, perform rolling updates, and know when the service is healthy enough to receive traffic.

* **5. Microservice Architecture:** The service is a small, single-purpose application. It exposes its functionality via a well-defined API contract (the [OpenAPI spec](https://www.google.com/search?q=api/geoTree.yaml)), making it a perfect example of a microservice that can run as part of a larger ecosystem.

* **6. Full CI/CD Automation:** The project is fully automated with GitHub Actions for:

    * **CI:** Automatic testing (`go.yml`).
    * **Security:** Container vulnerability scanning (`trivy-scan.yml`).
    * **CD:** Automated container builds and publishing (`docker-publish.yml`).
