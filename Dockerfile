...
ARG APP_REVISION
ARG BUILD
ARG APP_REPOSITORY=https://github.com/lao-tseu-is-alive/go-geo-tree-table

# Add Maintainer Info
LABEL maintainer="cgil"
LABEL org.opencontainers.image.title="go-geo-tree-table"
...
LABEL org.opencontainers.image.url="https://ghcr.io/lao-tseu-is-alive/go-geo-tree-table:latest"
...
COPY "cmd/geoTreeTableServer" ./geoTreeTableServer
COPY pkg ./pkg

# Clean the APP_REPOSITORY for ldflags
RUN APP_REPOSITORY_CLEAN=$(echo $APP_REPOSITORY | sed 's|https://||') && \
    CGO_ENABLED=0 GOOS=linux go build -a -ldflags="-w -s -X ${APP_REPOSITORY_CLEAN}/pkg/version.REVISION=${APP_REVISION} -X ${APP_REPOSITORY_CLEAN}/pkg/version.BuildStamp=${BUILD}" -o geoTreeTableServer ./geoTreeTableServer
...
LABEL org.opencontainers.image.description="Using the Go programming language to develop a backend API and a frontend interface for creating, managing, and importing tree location data in Lausanne, Switzerland"
LABEL org.opencontainers.image.url="ghcr.io/lao-tseu-is-alive/go-geo-tree-table:latest"
LABEL org.opencontainers.image.source="https://github.com/lao-tseu-is-alive/go-geo-tree-table"
...
COPY --from=builder /app/geoTreeTableServer .
...
# No HEALTHCHECK here: FROM scratch has no shell or curl.
# Health monitoring is handled by Kubernetes httpGet probes on /health and /readiness.

# Command to run the executable
CMD ["./geoTreeTableServer"]
