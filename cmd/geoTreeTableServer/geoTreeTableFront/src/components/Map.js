...
export async function getWmtsLausanneBaseLayers(url = urlLausanneMN95, initialBaseLayer = "fonds_geo_osm_bdcad_couleur") {
    const arrWmtsLayers = [];
    try {
        const WMTSCapabilities = await getWMTSCapabilitiesFromUrl(url);
        const WMTSCapabilitiesParsed = parser.read(WMTSCapabilities);
        arrWmtsLayers.push(createBaseOlLayerTile(WMTSCapabilitiesParsed, "Orthophoto 2016 (Lausanne)", "orthophotos_ortho_lidar_2016", initialBaseLayer === "orthophotos_ortho_lidar_2016"));
        arrWmtsLayers.push(createBaseOlLayerTile(WMTSCapabilitiesParsed, "Fond cadastral (Lausanne)", "fonds_geo_osm_bdcad_gris", initialBaseLayer === "fonds_geo_osm_bdcad_gris"));
        arrWmtsLayers.push(createBaseOlLayerTile(WMTSCapabilitiesParsed, "Plan ville (Lausanne)", "fonds_geo_osm_bdcad_couleur", initialBaseLayer === "fonds_geo_osm_bdcad_couleur"));
...
async function getWmtsSwissTopoBaseLayers(url, initialBaseLayer) {
    const arrWmtsLayers = [];
    try {
        const WMTSCapabilities = await getWMTSCapabilitiesFromUrl(url);
        const WMTSCapabilitiesParsed = parser.read(WMTSCapabilities);
        arrWmtsLayers.push(createBaseOlLayerTile(WMTSCapabilitiesParsed, "Orthophoto SWISSIMAGE", "ch.swisstopo.swissimage", initialBaseLayer === "ch.swisstopo.swissimage"));
        arrWmtsLayers.push(createBaseOlLayerTile(WMTSCapabilitiesParsed, "National Map 1:10&#39;000 (color)", "ch.swisstopo.landeskarte-farbe-10", initialBaseLayer === "ch.swisstopo.landeskarte-farbe-10"));
        arrWmtsLayers.push(createBaseOlLayerTile(WMTSCapabilitiesParsed, "National Map 1:10&#39;000 (grey)", "ch.swisstopo.landeskarte-grau-10", initialBaseLayer === "ch.swisstopo.landeskarte-grau-10"));
        return arrWmtsLayers;
    }
    catch (err) {
 ...
 */
export const getLayerByName = (olMap, layerName) => {
    ...
    for (const layer of allLayers) {
        if (localDebug)
            log.l(`## in getLayerByName : layer) : [${typeof layer}] ${layer}`, layer, layer.getProperties());
...
export const addMarker2Layer = (olMap, layerName, clearLayer = false, marker) => {
...
export const getPointStyle = (feature, resolution) => {
    const localDebug = false;
    if (localDebug)
...
export const getPolygonStyle = (feature, resolution) => {
    const options = {
    ...
    const localDebug = false;
    if (localDebug)
        log.t("## Entering getPolygonStyle with feature :", feature);
    if (localDebug)
...
const getVectorSourceGeoJson = (geoJsonData) => {
 ...
 */
export const addGeoJsonLayer = (olMap, layerName, geoJsonData, clusterDistance = defaultClusterDistance, clusterMinDistance = defaultClusterMinDistance) => {
    ...
    if (!isNullOrUndefined(olMap)) {
        if (!isNullOrUndefined(geoJsonData)) {
            ...
            if (olLayer == null) {
                ...
                const oldVectorSource = olLayer.getSource();
 ...
 */
export async function createSwissMap(divOfMap, centerOfMap = lausanneGare, zoomLevel = 16, baseLayer = defaultBaseLayer) {
