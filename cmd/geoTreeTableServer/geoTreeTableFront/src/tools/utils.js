...
export const getDateFromTimeStamp = (isoDate) => {
    log.t(`#> entering : ${isoDate}`, isoDate);
    if (typeof isoDate !== "string")
        return "not_date";
    if (isNullOrUndefined(isoDate) || isoDate.indexOf("T") < 0)
        return "";
    const dateTS = dateIso2Fr(isoDate.split("T")[0]);
    log.l(`dateTS : ${dateTS}`, isoDate);
    return dateTS;
};
export const getDateIsoFromTimeStamp = (isoDate) => {
    log.t(`#> entering : ${isoDate}`, isoDate);
    if (typeof isoDate !== "string")
        return "not_date";
...
export const truncateText = (text, maxSize = 40) => {
...
export const parseJsonWithDetailedError = (jsonString, context = 100) => {
...
export const extractFirstMatch = (str, regex) => {
